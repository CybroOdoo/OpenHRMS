...
class HrLoanAcc(models.Model):
    ...
    move_count = fields.Integer(string="Journal Entries", compute='_compute_move_count', compute_sudo=True)
    ...
    def _compute_move_count(self):
        ...
        for loan in self:
            if loan.id:
                loan.move_count = self.env['account.move'].sudo().search_count([('line_ids.loan_id', '=', loan.id)])
            else:
                loan.move_count = 0
    ...
    def action_view_journal_entries(self):
        ...
        """
        self.ensure_one()
        moves = self.env['account.move'].sudo().search([('line_ids.loan_id', '=', self.id)])
