...
class ResourceCalendar(models.Model):
    ...
    def _compute_two_weeks_attendance(self):
        """Filter out dummy line_section records to prevent blank rows in 2-week calendar views."""
        super()._compute_two_weeks_attendance()
        for calendar in self:
            if calendar.two_weeks_calendar:
                calendar.attendance_ids_1st_week = calendar.attendance_ids_1st_week.filtered(
                    lambda a: not a.display_type
                )
                calendar.attendance_ids_2nd_week = calendar.attendance_ids_2nd_week.filtered(
                    lambda a: not a.display_type
                )

    def _get_two_weeks_attendance(self):
        """Exclude dummy line_section attendances that appear as blank rows."""
        attendances = super()._get_two_weeks_attendance()
        cleaned = []
        for att in attendances:
            if isinstance(att, tuple) and len(att) > 2 and isinstance(att[2], dict):
                if att[2].get('display_type') == 'line_section':
                    continue
            cleaned.append(att)
        return cleaned
