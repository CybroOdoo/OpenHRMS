...
class SalaryAdvance(models.Model):
    ...
    monthly_wage = fields.Monetary(string="Monthly Wage", compute='_compute_advance_limits', store=True, compute_sudo=True,
                                   help="The monthly wage of the employee based on their active contract.")
    maximum_allowed = fields.Monetary(string="Maximum Allowed", compute='_compute_advance_limits', compute_sudo=True,
                                      help="The maximum advance amount allowed based on company policy.")
    already_outstanding = fields.Monetary(string="Already Outstanding", compute='_compute_advance_limits', compute_sudo=True,
                                          help="The total amount of unsettled advances currently held by this employee.")
    eligible_amount = fields.Monetary(string="Remaining Eligible", compute='_compute_advance_limits', compute_sudo=True,
    ...
    payment_count = fields.Integer(string="Payment Count", compute='_compute_payment_count', compute_sudo=True,
                                   help="Number of accounting payments linked to this advance.")
    move_ids = fields.Many2many('account.move', string="Journal Entries", compute='_compute_move_ids', compute_sudo=True,
                                help="Journal entries generated for this advance.")
    move_count = fields.Integer(string="Move Count", compute='_compute_move_count', compute_sudo=True,
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if 'payment_ids' in vals and not vals['payment_ids']:
                vals.pop('payment_ids')
    ...
    def write(self, vals):
        if 'payment_ids' in vals and not vals['payment_ids']:
            vals.pop('payment_ids')
        return super(SalaryAdvance, self).write(vals)
    ...
    def _compute_move_ids(self):
        ...
        for rec in self:
            rec.move_ids = rec.payment_ids.sudo().mapped('move_id')
