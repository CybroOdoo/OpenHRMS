...
import { onWillStart, proxy, onWillUnmount } from "@odoo/owl";
...
export class HrDashboard extends Component{
    ...
    showCustomToast(message, type = "success") {
        ...
        let iconHtml = '';
        if (type === 'success') {
            iconHtml = `<div style="width: 24px; height: 24px; border-radius: 50%; background-color: #dcfce7; color: #16a34a; display: flex; align-items: center; justify-content: center;"><i class="oi" data-icon="check" style="font-size: 12px;"></i></div>`;
        } else {
            iconHtml = `<div style="width: 24px; height: 24px; border-radius: 50%; background-color: #fee2e2; color: #dc2626; display: flex; align-items: center; justify-content: center;"><i class="oi" data-icon="close" style="font-size: 12px;"></i></div>`;
        }
   ...
   async hr_contract() {
    ...
    employee_broad_factor() {
        ...
        this.action.doAction({
            ...
            domain: domain,
            target: 'current',
            context:{'order':'number_of_days desc'}
        })
    }
    ...
    async fetch_data() {
        ...
        if (this.state.is_manager) {
            var pendingLeaves = await this.orm.searchRead('hr.leave', [['state', 'in', ['confirm', 'validate1']]], ['employee_id', 'work_entry_type_id', 'request_date_from', 'request_date_to', 'number_of_days']);
            ...
            this.state.pending_approvals = pendingLeaves.map((l, i) => {
                ...
                let type = l.work_entry_type_id ? l.work_entry_type_id[1] : (l.holiday_status_id ? l.holiday_status_id[1] : 'Leave');
