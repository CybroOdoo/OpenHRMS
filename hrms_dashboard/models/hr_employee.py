...
class HrEmployee(models.Model):
    ...
    def get_user_employee_details(self, date_from=None, date_to=None):
        ...
        uid = getattr(request.session, 'uid', None) if (request and hasattr(request, 'session')) else self.env.uid
        user = request.env.user if request and hasattr(request, 'env') else self.env.user
        ...
        leave_type_field = 'work_entry_type_id' if 'work_entry_type_id' in self.env['hr.leave']._fields else 'holiday_status_id'
        leaves = self.env['hr.leave'].sudo().search_read(
            leave_domain,
            fields=['request_date_from', 'request_date_to', 'state', leave_type_field])
        for line in leaves:
            leave_type = line.pop(leave_type_field, False)
            line['type'] = leave_type[1] if leave_type and isinstance(leave_type, (list, tuple)) else (leave_type or 'Leave')
        ...
        if not self.env.user.has_group('hr.group_hr_manager'):
            ts_domain.append(('employee_id', '=', employee[0]['id']))
    ...
    def get_upcoming(self, date_from=None, date_to=None):
        ...
        if date_from and date_to:
            ...
            ann_domain.append(('date_end', '>=', date_from))
        elif date_to:
            ann_domain.append(('date_start', '<=', date_to))
        else:
            ann_domain.extend([
        ...
        or_conditions = [('is_announcement', '=', True)]
        if employee:
            or_conditions.append(('employee_ids', 'in', employee.id))
        ...
        ann_domain.extend(or_domain)
        announcements = []
        if 'hr.announcement' in self.env:
            announcements = self.env['hr.announcement'].sudo().search_read(
                ann_domain, fields=['announcement_reason', 'date_start', 'date_end', 'create_date', 'create_uid'])
    ...
    def get_work_days_dashboard(self, from_datetime, to_datetime,
        ...
        if not calendar:
            return max(1.0, float((to_datetime.date() - from_datetime.date()).days))
        try:
            data = calendar.get_work_duration_data(from_datetime, to_datetime, compute_leaves=compute_leaves, domain=domain)
...
class HrEmployeePublic(models.Model):
    """ Inherit hr.employee.public to allow public profile access to birthday. """
    _inherit = 'hr.employee.public'

    birthday = fields.Date(string='Date of Birth', readonly=True,
                           help="Birthday of employee")
