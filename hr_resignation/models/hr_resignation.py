...
class HrResignation(models.Model):
    ...
    def default_get(self, fields_list):
        """Pre-populate joined_date and contract details using sudo to avoid field AccessError for normal users."""
        res = super(HrResignation, self).default_get(fields_list)
        employee_id = res.get('employee_id') or self.env.user.employee_id.id
        if employee_id:
            employee = self.env['hr.employee'].sudo().browse(employee_id)
            if 'joined_date' in fields_list and not res.get('joined_date'):
                res['joined_date'] = employee.joining_date
            if 'notice_period' in fields_list and not res.get('notice_period'):
                ...
                if contract:
                    ...
                    if 'employee_contract' in fields_list:
                        res['employee_contract'] = contract.contract_template_id.name
        return res
    ...
    def _onchange_employee_id(self):
        ...
        today = fields.Date.today()
        for rec in self:
            rec.joined_date = rec.employee_id.sudo().joining_date if rec.employee_id else False
    ...
    def create(self, vals):
        ...
        for v in vals_list:
            ...
            if v.get('employee_id') and not v.get('joined_date'):
                employee = self.env['hr.employee'].sudo().browse(v['employee_id'])
                if employee:
                    v['joined_date'] = employee.joining_date
    ...
    def action_reset_to_draft(self):
        ...
        for resignation in self:
            ...
            resignation.sudo().employee_id.write({
                'active': True,
                'resigned': False,
                'fired': False,
            })
    ...
    def action_relieve(self):
        ...
        for resignation in self:
            ...
            resignation.sudo().employee_id.write({
                'active': False,
                'resign_date': resignation.expected_revealing_date,
                'resigned': resignation.reason_category_id.name == 'Resigned',
                'fired': resignation.reason_category_id.name == 'Fired',
                'departure_reason_id': resignation.reason_category_id.id if resignation.reason_category_id else False,
                'departure_date': resignation.relieving_date,
            })

            if resignation.employee_id.user_id:
                resignation.sudo().employee_id.user_id.write({'active': False})
                resignation.sudo().employee_id.write({'user_id': False})
